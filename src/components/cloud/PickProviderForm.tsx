...
type PickProviderFormProps = {
  error?: string;
  location?: string;
  onSelectLocation: (location: string) => void;
};
const PickProviderForm: FC<PickProviderFormProps> = ({
  error,
  location,
  onSelectLocation,
}) => {
  ...
  const fetchData = async () => {
    try {
      const result = await api.get<LocationType[]>("/providers");
  ...
  const handleRemoveLocation = async (_id: string) => {
    try {
      const { isConfirmed } = await Swal.fire({
        title: `<p class="leading-tight">Bạn có muốn mua xóa Provider
         này</p>`,
        icon: "success",
      ...
      if (isConfirmed) {
        await api.delete(`/providers/${_id}`);
        fetchData();
  ...
  const handleEditLocation = async (_id: string, value: string) => {
    try {
      const { isConfirmed } = await Swal.fire({
        title: `<p class="leading-tight">Bạn có muốn mua sửa Provider này</p>`,
        icon: "success",
        showCancelButton: true,
        confirmButtonColor: "#1DC071",
        cancelButtonColor: "#d33",
      ...
      if (isConfirmed) {
        await api.patch(`/providers/${_id}`, { name: value });
        fetchData();
      ...
      <DropdownWithComponents.Select
        ...
        placeholder={
          location ? (
            ...
            <span className="text-text4">Select Provider</span>
          )
        }
      ></DropdownWithComponents.Select>
        ...
        {locations.length > 0 &&
          ...
          locations.map((item) => (
            <DropdownWithComponents.Option
              key={uuidv4()}
              onClick={() => onSelectLocation(item._id)}
            >
...
export default PickProviderForm;
